'''
Based on Github kubeflow/examples/jpx-tokyo-stock-exchange-kaggle-competition/helper-files/local_api.py
'''
...
class iter_test():
    ...
    def __next__(self):
        if self.idx == len(self.dates):
            ...
            sample_submission['Rank'] = range(len(sample_submission))
            self.idx += 1
...
class local_api():
    def __init__(self, data_dir, env_type = 'valid', portfolio_size = 200, toprank_weight_ratio = 2):
        """
        ...
        Parameters:
            ...
            env_type: either 'valid' or 'test'. 'valid' is used for validation set, and 'test' is used for the testing set.
            portfolio_size (int): # of equities to buy/sell
            toprank_weight_ratio (float): the relative weight of the most highly ranked stock compared to the least.
        """
        if env_type == 'valid':
        ...
        self.toprank_weight_ratio = toprank_weight_ratio
    def make_env(self):
        return self
    def iter_test(self):
        self.submission = []
    ...
    def score(self):
        return calc_spread_return_sharpe(self.submission.merge(self.gt_prices), self.portfolio_size, self.toprank_weight_ratio)[0]